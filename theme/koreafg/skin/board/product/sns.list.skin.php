<?
function sns_list_share_icon($url, $title, $img='', $icon='', $eol='') {
	...
	$sns = array();
	$sns[] = array('facebook', 'facebook', 'Facebook','페이스북','facebook');
	$sns[] = array('googleplus', 'gplus', 'GooglePlus','구글 플러스','google-plus');
	$sns[] = array('twitter', 'twitter', 'Twitter','트위터','twitter');
	//$sns[] = array('kakaostory', 'kakaostory', 'KakaoStory','카카오스토리');
	//$sns[] = array('kakaotalk', 'kakaotalk', 'KakaoTalk','카카오톡','kakao');
	//$sns[] = array('naverband', 'naverband', 'NaverBand','네이버 밴드');
	//$sns[] = array('naver', 'naver', 'Naver','네이버');
	//$sns[] = array('tumblr', 'tumblr', 'Tumblr','텀블러');
	$sns[] = array('pinterest', 'pinterest', 'Pinterest','핀터레스트','pinterest-p');
	...
	for($i=0; $i < $sns_cnt; $i++) {
		...
		if($sns[$i][0] == 'kakaotalk'){
			...
			$thumb = ($img) ? sns_list_thumbnail($img, 300, 0) : array('src'=>'', 'height'=>'');
			$sns_icon .= '<li class="'.$sns[$i][4].'"><a href="'.$sns_href.'" onclick="kakaolink_send(\''.$sns_msg.'\',\''.$sns_url.'\',\''.$thumb['src'].'\', \'300\', \''.$thumb['height'].'\'); return false;" target="_blank">';
			$sns_icon .= '<i class="fa fa-'.$sns[$i][4].'"></i></a></li>'.$eol;
		} else {
			$sns_icon .= '<li class="'.$sns[$i][0].'"><a href="'.$sns_href.'" onclick="send_list_sns(\''.$sns[$i][0].'\',\''.$sns_href.'\'); return false;">';
			$sns_icon .= '<i class="fa fa-'.$sns[$i][4].'"></i></a></li>'.$eol;
		}
	}
...
// 썸네일 생성
function sns_list_thumbnail($url, $thumb_width, $thumb_height, $is_create=false, $is_crop=true, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3') {
