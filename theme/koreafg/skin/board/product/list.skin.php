...
include $board_skin_path.'/sns.list.skin.php';
?>
...
<div id='board-container'>
	...
	<div class='product-item-wrap'>
		...
		<div class="product-items col-lg-<?=$cols?> col-md-4 <?=$ca_name?>">
			<?
			if ($is_checkbox) {
				$idx=$list[$i][wr_id];
			...
			<div class="product-item">
				<div class="product-img img-over" data-img='<?=$thumb[ori]?>' data-url='<?=$list[$i][href]?>'>
					<a href="<?=$list[$i][href]?>">
						<? if($thumb[src]){ ?>
						<img src="<?=$thumb[src]?>">
						<? }else{ ?>
				...
				<div class="product-info">
					<a href="<?=$list[$i][href]?>" class="product-title"><?=$list[$i][wr_subject]?></a>
					<div class="pro-rating">
						<?=$list[$i][wr_2]?>
					</div>
					<h6 class="pro-price"><?=number_format($list[$i][wr_3])?>원</h6>
					<ul class="action-button">
						<?=sns_list_share_icon($list[$i]['href'], $list[$i]['subject'], $thumb[ori]);?>
					</ul>
				</div>
			</div>
		</div>
