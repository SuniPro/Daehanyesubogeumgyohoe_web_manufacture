...
<div id='board-container'>
	...
	<div class='blog-item-wrap board-list-top-line'>
		<div class='row blog-item-row'>
		<?
		for ($i=0; $i<count($list); $i++) {
			$ca_name=md5($list[$i]['ca_name']);
			
			if($list[$i][wr_movie_url] && $list[$i][wr_use_movie_image]){
				$thumb[src]=sw_get_video_tags($list[$i][wr_movie_url],"100%","100%");
				$thumb[ori]="movie";
			}else{
				if($list[$i][wr_thumb]){
		...
		<div class="blog-items col-lg-<?=$cols?> <?=$ca_name?>">
			<?
			if ($is_checkbox) {
				$idx=$list[$i][wr_id];
			...
			<? }else{ ?>
			<div class='blog-item-loading-wrap'>
				<div class="blog-spinner">
					<div class="skin-loader blog-rect1"></div>
					<div class="skin-loader blog-rect2"></div>
					<div class="skin-loader blog-rect3"></div>
					<div class="skin-loader blog-rect4"></div>
					<div class="skin-loader blog-rect5"></div>
				</div>
			</div>
			<div class="blog-item img-over" data-img='<?=$thumb[ori]?>' data-url='<?=$list[$i][href]?>'>
			...
			<div class="blog-info">
				<div class='blog-info-date'>
					<div class='blog-date'>
						<p class='blog-date-md'><?=date('m-d',strtotime($list[$i][wr_datetime]))?></p>
						<p class='blog-date-y'><?=date('Y',strtotime($list[$i][wr_datetime]))?></p>
					</div>
				</div>
				<div class="blog-info-item">
					...
					<div class="blog-u-info">
						<div class="u-info">
							Posted by <span><?=$list[$i][wr_name]?></span>
						</div>
						<p class="u-info-links">
							...
							<span class="separator"> - </span>
							<? } ?>
							<span class="u-info-date"><i class='fa fa-clock-o'></i> <?=date('H:i:s',strtotime($list[$i][wr_datetime]))?></span>
						</p>
					</div>
					<div class="wb-desc">
						<p><?=cut_str(strip_tags($list[$i][content]),350)?></p>
					</div>
					<div class="wb-icons">
						<ul>
							<li><i class='fa fa-comment-o'></i> <?=number_format($list[$i]['wr_comment'])?></li>
							<? if ($is_good){ ?>
							<li><i class='fa fa-thumbs-o-up'></i> <?=$list[$i]['wr_good']?></li>
							<? } ?>
							<? if ($is_nogood){ ?>
							<li><i class='fa fa-thumbs-o-down'></i> <?=$list[$i]['wr_nogood']?></li>
							<? } ?>
							<li><i class='fa fa-eye'></i> <?=$list[$i]['wr_hit']?></li>
						</ul>
					</div>
				</div>
			</div>
